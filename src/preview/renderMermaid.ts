...
let mermaidLoader: MermaidLoader = () => import("@/preview/loadMermaid").then((m) => m.loadMermaid());
let renderSeq = 0;
...
export async function renderMermaidInRoot(
  ...
  if (options?.generation === undefined) {
    generation = ++renderSeq;
  } else {
    generation = options.generation;
    // Adopt the caller's generation so PreviewPane and this module share one
    // cancellation timeline for overlapping html updates.
  ...
  for (let index = 0; index < blocks.length; index += 1) {
    ...
    try {
      ...
      if (generation !== renderSeq || !pre.isConnected) {
        return false;
      }
