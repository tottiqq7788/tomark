...
<template>
  <div class="help-body" data-testid="help-settings-panel">
    ...
    <section>
      <h3>源码粘贴图片</h3>
      <ul>
        <li>在左侧源码区粘贴剪贴板图片时，会写入文档旁 <code>assets/</code> 目录，并插入相对路径引用。</li>
        <li>未命名文档会先弹出「另存为…」；取消则不写盘、不改源码。</li>
        <li>macOS 截图等场景若 Web 剪贴板拿不到图片文件，会在同一次粘贴手势内尝试只读原生回退并保存为 PNG。</li>
        <li>预览中的图片仍为只读查看，不会因此变为可编辑。</li>
      </ul>
    </section>
    ...
    <section>
      ...
      <ul>
        ...
        <li>
          行内代码、围栏代码块、图片、脚注生成内容、自动链接和 Mermaid
          在右侧不可键入；显式链接的 URL / title 也只能在源码区修改。任务复选框不可键入，但可点击切换勾选并写回源码。
        </li>
        <li>无法可靠映射回源码的内容会保持只读；需要修改时请使用左侧源码区。</li>
        <li>
          链接普通点击用系统默认浏览器打开安全外链；<kbd>Cmd</kbd> /
          <kbd>Ctrl</kbd>+点击仍定位源码。也可使用浮动工具条「打开链接」，或
          <kbd>Alt</kbd> / <kbd>Option</kbd>+点击打开。
        </li>
        <li>
