...
- Mermaid 是正式能力：`mermaid` 围栏在预览中渲染但保持只读，HTML/PNG 导出也应保留图表结果；普通点击成功图显示浮动工具栏（全屏 icon、复制源码 icon、英文 SVG、英文 PNG 白底 2×），不提供定位源码按钮；Cmd/Ctrl+点击仍定位源码，不得从预览修改围栏源码；复制源码不等于批准编辑。
- HTML/PNG 导出是正式能力：支持嵌入图片 HTML、资源目录 HTML 与长图 PNG，输入为当前编辑器内容（含未保存修改）。
