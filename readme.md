...
- **明确的只读边界**：行内代码、围栏代码块、图片、脚注生成内容、自动链接和 Mermaid 在右侧不可键入；显式链接的 URL / title 也只能在源码区修改。无法可靠映射回源码的内容会保持只读。任务复选框不可键入，但可点击切换勾选。可安全往返的 flowchart Mermaid 允许受限可视化编辑写回。
- **任务复选框**：预览中点击 ☐/☑ 可切换并写回源码 `[ ]`/`[x]`；Cmd/Ctrl+点击仍定位源码。
- **Mermaid 图表**：将 `mermaid` 围栏渲染为图表，并在 HTML/PNG 导出中保留图表结果；点击图表可显示工具栏（可编辑 flowchart 前置编辑；全屏、复制源码、复制图片、SVG、PNG）；Cmd/Ctrl+点击仍定位源码。编辑弹窗草稿仅本地，保存后写回围栏正文。
- **图片查看**：预览真实渲染 Markdown 图片（含本地相对路径）；点击可显示工具栏（全屏、复制图片、PNG）；Cmd/Ctrl+点击仍定位源码。
- **源码粘贴图片**：在左侧源码区粘贴剪贴板图片时，未命名文档先另存为，再将图片写入文档旁 `assets/` 并插入相对路径引用；macOS 截图等场景在 Web 读取失败时可走只读原生回退；预览侧图片仍只读。
- **HTML/PNG/PDF 导出**：使用当前编辑器内容（包括未保存修改）导出嵌入图片 HTML、资源目录 HTML、长图 PNG 或长图不分页 PDF（位图单页，不可检索文字；超高单页在部分阅读器体验较差属已定权衡）。
- **链接交互**：普通点击安全链接用系统默认浏览器打开；Cmd/Ctrl+点击仍定位源码。浮动工具条「打开链接」或 Alt/Option+点击亦可打开。
