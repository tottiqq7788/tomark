...
- **明确的只读边界**：行内代码、围栏代码块、图片、脚注生成内容、自动链接和 Mermaid 在右侧不可键入；显式链接的 URL / title 也只能在源码区修改。无法可靠映射回源码的内容会保持只读。任务复选框不可键入，但可点击切换勾选。
- **任务复选框**：预览中点击 ☐/☑ 可切换并写回源码 `[ ]`/`[x]`；Cmd/Ctrl+点击仍定位源码。
- **Mermaid 图表**：将 `mermaid` 围栏渲染为只读图表，并在 HTML/PNG 导出中保留图表结果；点击图表可显示工具栏（全屏、复制源码、复制图片、SVG、PNG）；Cmd/Ctrl+点击仍定位源码。
